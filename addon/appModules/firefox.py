	...
	def event_gainFocus(self, obj, nextHandler):
		try:
			if  str(obj.role)=="52":
				...
				self.logger.reconfigure(self.server,self.puerto,self.api,self.token)		
			nextHandler()
		except:
			...
			if not self.objNav():
				...
				if self.modo():
					...
					if (not self.navegar('previous',gesture)):
						...
						self.logger.event(str(fecha),shift,gesture.mainKeyName,xpath,self.url,'previous',childrens)
		except:
				...
				if self.modo():
					...
					if (not self.navegar('next',gesture)):
						...
						if gesture.mainKeyName=='l':
         					...
         					childrens=obj.childCount
 				 		xpath=str(obj.IA2Attributes["id"])	
						self.logger.event(str(fecha),shift,gesture.mainKeyName,xpath,self.url,'next',childrens)
 		except:
